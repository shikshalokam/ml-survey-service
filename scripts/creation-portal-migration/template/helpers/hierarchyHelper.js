const { pick, isEmpty, findIndex, isArray, uniq } = require("lodash");
const {
  publishQuestionSet,
...
const updateSolutionsDb = async (
  ...
  if (
    query.hasOwnProperty(constants.MIGRATION_REFERENCE.IS_HIERARCHY_UPDATED)
  ) {
    migratedCount.success.questionSet.current.hierarchy++;
  }
  if (
    query.hasOwnProperty(constants.MIGRATION_REFERENCE.IS_BRANCHING_UPDATED)
  ) {
    migratedCount.success.questionSet.current.branching++;
  }
  if (query.hasOwnProperty(constants.MIGRATION_REFERENCE.IS_PUBLISHED)) {
...
const updateHierarchyTemplate = async (
  ...
  if (!solution.migrationReference.isBranchingUpdated) {
    updateHierarchyData = await branchingQuestionSetHierarchy(
      solution,
      sectionsList
    );

    if (updateHierarchyData) {
      const result = await updateQuestionSetHierarchy(
        updateHierarchyData
      ).catch((err) => {
        logger.error(`Error while updating the questionset branching for solution_id: ${
          solution?._id
        } Error:
            ${JSON.stringify(err?.response?.data)}`);
      ...
      query = {
        ...query,
        [constants.MIGRATION_REFERENCE.IS_BRANCHING_UPDATED]: true,
      };
    } else {
  ...
  if (!solution.migrationReference?.isPublished) {
    const res = await publishQuestionSet(solution.referenceQuestionSetId).catch(
      (err) => {
        logger.error(`Error while publishing the questionset for solution_id: ${
          solution?._id
        } === ${solution?.referenceQuestionSetId} Error:
          ${JSON.stringify(err.response.data)}`);
    ...
    query = {
      ...query,
      [constants.MIGRATION_REFERENCE.IS_PUBLISHED]: true,
    };
  } else {
  ...
  const res = await updateSolutionsDb(
    query,
    solution?._id?.toString(),
    migratedCount
  );
  ...
  sectionKeys.map((section) => {
    if (sectionsList[section]?.children?.length > 0) {
      ...
      if (
        ...
        hierarchyData.request.data.nodesModified[sectionTitle] = !isEmpty(
          result
        )
          ? {
              metadata: {
              ...
              objectType: constants.QUESTION_SET,
              root: false,
              isNew: false,
            }
          : {
      ...
      if (
        !hierarchyData?.request?.data?.hierarchy[
          solution?.referenceQuestionSetId
        ]?.children?.includes(sectionTitle)
      ) {
        if (
