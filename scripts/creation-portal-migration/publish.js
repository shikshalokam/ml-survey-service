  ...
  try {
    ...
    for (const row of csv) {
      ...
      if (referenceQuestionId && !isPublished) {
        ...
        const res = await publishQuestion(referenceQuestionId).catch((err) => {
          ...
          if (
            !migratedCount.failed.question.ids.includes(referenceQuestionId)
          ) {
        ...
        if (res?.responseCode === httpStatusCode.ok.code) {
          ...
          logger.info(
            `createQuestion Template published: ${referenceQuestionId}`
          );
        }
      }
      ...
      if (referenceQuestionId) {
        ...
        query = {
          referenceQuestionId,
          [constants.MIGRATION_REFERENCE.IS_PUBLISHED]: isPublished,
        };
      } else {
        query = {
          ...query,
    ...
    for (const row of csv) {
      ...
      if (referenceQuestionSetId && !isPublished) {
        ...
        const res = await publishQuestionSet(
      ...
      const result = await updateSolutionsDb(
        query,
        solution?._id?.toString(),
        migratedCount
      );
