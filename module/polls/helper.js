    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let pollDocument = {
          ...
          startDate: new Date(),
          endDate: new Date(
            new Date().setDate(new Date().getDate() + pollData.endDate)
          ),
        ...
        for (
          let question = 0;
          question < pollData.questions.length;
          question++
        ) {
          let options = [];
          ...
          if (pollData.questions[question].options.length > 0) {
            for (
              let option = 0;
              option < pollData.questions[question].options.length;
              option++
            ) {
              options.push({
                value: "R" + (option + 1),
        ...
        let getPollLink = await this.getPollLinkById(
          createPollResult._id,
          appName
        );
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!updateResponse.success && !updateResponse.data) {
          throw new Error(
            messageConstants.apiResponses.POLL_COULD_NOT_BE_DELETED
          );
        }
  ...
  static getPollQuestions(pollId = "", appName = "") {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let pollLink = await this.getPollLinkById(pollId, appName);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (new Date() > new Date(pollQuestions[0].endDate)) {
          if (
            pollQuestions[0].status == messageConstants.common.ACTIVE_STATUS
          ) {
            await this.updatePollDocument(
              { link: link },
          ...
          let pollSubmissionDocument =
            await pollSubmissionDocumentHelper.pollSubmissionDocuments(
              {
                pollId: pollQuestions[0]._id,
                userId: userId,
              },
  ...
  static report(pollId = "") {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let pollDocument = await this.pollDocuments({ _id: pollId }, [
          "result",
          "questions",
          "numberOfResponses",
        ]);
        ...
        if (pollDocument[0].numberOfResponses == 0) {
          throw new Error(
            messageConstants.apiResponses.POLL_SUBMISSIONS_NOT_FOUND
          );
        }
        ...
        Object.keys(result).forEach((singleQuestion) => {
          ...
          let groupOptionsByValue = _.keyBy(
            groupByQuestionId[singleQuestion].options,
            "value"
          );
          ...
          for (let value in result[singleQuestion]) {
            ...
            report.series[0].data.push({
              name: label,
              y:
                (result[singleQuestion][value] /
                  pollDocument[0].numberOfResponses) *
                100,
            });
          }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (Object.keys(query).length == 0) {
          throw new Error(
            messageConstants.apiResponses.POLL_UPDATE_QUERY_REQUIRED
          );
        }
        ...
        if (updateResponse.nModified == 0) {
          throw new Error(
            messageConstants.apiResponses.POLL_COULD_NOT_BE_UPDATED
          );
        }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let pollLink = await this.pollDocuments(
          { _id: pollId, endDate: { $gt: new Date() } },
          ["link"]
        );
        ...
        return resolve({
          ...
          data:
            appsPortalBaseUrl +
            appName +
            messageConstants.common.TAKE_POLL +
            pollLink[0].link,
        });
      } catch (error) {
        return resolve({
