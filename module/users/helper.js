...
const entityAssessorsHelper = require(MODULES_BASE_PATH +
  "/entityAssessors/helper");
      ...
      try {
        ...
        if (observationsData.length > 0) {
          observationsData.forEach((observation) => {
            ...
            observationEntities = observationEntities.concat(
        ...
        let programs = await programsHelper.programDocument(programIds, [
          "name",
          "externalId",
          "description",
        ]);
        ...
        let removedSolutions =
          await userExtensionsHelper.userExtensionDocuments(
            {
              userId: userId,
            },
        ...
        for (
          let pointerToSolutionsArray = 0;
          pointerToSolutionsArray < solutions.length;
          pointerToSolutionsArray++
        ) {
          let solution = solutions[pointerToSolutionsArray];
          solution.showInHomeScreen = true;
          if (
            userRemovedSolutionsFromHomeScreen.length > 0 &&
            userRemovedSolutionsFromHomeScreen.indexOf(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (userDetails.surveySubmissions.length > 0) {
          surveySubmissions = _surveySubmissions(userDetails.surveySubmissions);
        }
        ...
        for (let user = 0; user < users.length; user++) {
          ...
          if (program && solution) {
            let programIndex = result.findIndex(
              (programData) => programData.externalId === program.externalId
            );
            ...
            let solutionIndex = result[programIndex].solutions.findIndex(
            ...
            if (solutionIndex < 0) {
              ...
              result[programIndex].solutions.push(solutionInformation);

              solutionIndex = result[programIndex].solutions.length - 1;
            }

            if (users[user].isSurvey) {
              if (surveySubmissions[users[user]._id]) {
                result[programIndex].solutions[solutionIndex].submissions =
                  surveySubmissions[users[user]._id]["submissions"];
              }
            } else {
              let solutionOrObservationId = users[user].isObservation
                ? users[user]._id
                : solution._id;

              let submissionData = users[user].isObservation
  ...
  static entities(userId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let result = {
          entityTypes: _entityTypesKeyValue(
            ...
            userData.entities.forEach((entity) => {
              ...
              if (entitiesData[entity.toString()]) {
                ...
                let entityExternalId =
                  entitiesData[entity.toString()]["metaInformation"][
                    "externalId"
                  ];

                let entityIndex = result.entities[
                  entitiesData[entity.toString()].entityType
                ].findIndex((entity) => entity.externalId === entityExternalId);

                if (entityIndex < 0) {
                  let entityInformation = _entityInformation(
                    userDetails.entitiesData[entity.toString()]
                  );
                  entityInformation["solutions"] = [];

                  result.entities[
                    entitiesData[entity.toString()].entityType
                  ].push(entityInformation);
                ...
                let solutionIndex = result.entities[
                  entitiesData[entity.toString()].entityType
                ][entityIndex].solutions.findIndex(
                  (solutionData) =>
                    solutionData._id.toString() === solution._id.toString()
                );

                if (solutionIndex < 0) {
                  let solutionOrObservationInformation = users[user]
                    .isObservation
                  ...
                  if (users[user].isObservation) {
                    ...
                    submission = _submissionInformation(
                      submissions,
                      solution._id,
                      entity
                    );
...
function _entityTypesKeyValue(entities) {
  ...
  entities.forEach((entity) => {
    let findEntityTypesIndex = result.findIndex(
...
function _observationSubmissions(observationSubmissions) {
  ...
  observationSubmissions.forEach((submission) => {
    ...
    submissions[submission.observationId.toString()][
      ...
      observationName:
        submission.observationInformation &&
      ...
      ratingCompletedAt: submission.ratingCompletedAt
...
function _observationSubmissionInformation(
  submissions,
  observationId,
  entityId
) {
...
function _solutionInformation(program, solution) {
  return {
    ...
    allowMultipleAssessemts: solution.allowMultipleAssessemts
      ...
      : false,
    showInHomeScreen: solution.showInHomeScreen
      ? solution.showInHomeScreen
      : false,
    isAPrivateProgram: solution.isAPrivateProgram
      ? solution.isAPrivateProgram
      : false,
...
function _entityInformation(entityDetails) {
  return {
    ...
    name: entityDetails.metaInformation.name
      ? entityDetails.metaInformation.name
      : "",
