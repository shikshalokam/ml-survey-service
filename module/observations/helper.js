...
module.exports = class ObservationsHelper {
   ...
   * @returns {Array} - Count of observations.
   */
    ...
    static countDocuments(findQuery = "all",) {
      return new Promise(async (resolve, reject) => {
        try {
          ...
          let countDocuments = await database.models.observations
  ...
  static create(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let userProfileData = {};
        let userProfile = await userProfileService.profile(
          requestingUserAuthToken,
          userId
        );
        ...
        if (addReportInfoToSolution && observationData.solutionId) {
          let updateSolution =
            await solutionHelper.addReportInformationInSolution(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          ...
          if (
            updatedUserProfile &&
            updatedUserProfile.success == true &&
            updatedUserProfile.profileMismatchFound == true
          ) {
  ...
  static listCommon(userId = "", sourceApi = "v2") {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (
          let pointerToEachObs = 0;
          pointerToEachObs < userObservations.length;
          pointerToEachObs++
        ) {
          if (userObservations[pointerToEachObs].entities)
            entities.push(...userObservations[pointerToEachObs].entities);
        }
        ...
        if (uniqueEntities.length > 0) {
          ...
          let entityDocument = await userProfileService.locationSearch(
            filterData,
            "",
            "",
            "",
        ...
        for (
          ...
          submissions.forEach((observationEntitySubmission) => {
            if (
              !observationEntitySubmissions[
                observationEntitySubmission.entityId
              ]
            ) {
              observationEntitySubmissions[
                observationEntitySubmission.entityId
          ...
          if (observationEntities.length > 0) {
            ...
            for (
              ...
              let observationEntity = entityDocuments.find(
                (entity) => entity.id == currentEntity
              );
              if (observationEntity) {
                ...
                observation.entities.push({
                  ...
                  submissionStatus: observationEntitySubmissions[
                    ...
                    ? observationEntitySubmissions[observationEntity.id]
                        .submissionStatus
                    : "pending",
  ...
  static getObservationLink(observationSolutionId, appName) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // will not validate Apps Details
  ...
  static verifyLink(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let result = await this.createObservation(dataObj, userId, solution);
      ...
      try {
        ...
        if (observationData[0]) {
          solutionData = await solutionHelper.solutionDocuments(
            ...
            ["allowMultipleAssessemts", "license"]
          );
          ...
          rootOrganisations = programsData[0].rootOrganisations
            ? programsData[0].rootOrganisations[0]
            : "";
        }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          ...
          let locationDeatails = gen.utils.filterLocationIdandCode(
            observationDocument[0].entities
          );
  ...
  static validateUserRole(bodyData, solutionId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (
          let roleCount = 0;
          roleCount < bodyData.role.split(",").length;
          roleCount++
        ) {
          ...
          const allowedEntityTypesForRole =
            await this.subEntityListBasedOnRoleAndLocation(bodyData, eachRole);

          //finding the entity type array with highest length
          if (
            allowedEntityTypesForRole.result &&
            allowedEntityTypesForRole.result.length >
  ...
  static subEntityListBasedOnRoleAndLocation(bodyData, role) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let findTargetedEntityIndex = subEntities.findIndex(
          (element) => element === targetedEntityType
        );
...
function _updateUserProfileBasedOnUserRoleInfo(
  userProfile,
  userRoleInformation
) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (userRoleInformation.role) {
        ...
        if (
          ...
          for (
            ...
            userProfile.profileUserTypes.length;
            pointerToCurrentProfileUserTypes++
          ) {
            ...
            if (
              currentProfileUserType.subType &&
              currentProfileUserType.subType !== null
            ) {
              ...
              if (
                !observation.userRoleInformation.role
                  .toUpperCase()
        ...
        for (
          ...
          const subRole =
            rolesInUserRoleInformation[pointerToRolesInUserInformation];
          // Check if userProfile.profileUserTypes exists and is an array of length > 0
          if (
            userProfile.profileUserTypes &&
            Array.isArray(userProfile.profileUserTypes) &&
            ...
            if (
              !_.find(userProfile.profileUserTypes, {
                type: subRole.toLowerCase(),
              }) &&
              !_.find(userProfile.profileUserTypes, {
      ...
      let userRoleInformationLocationObject = _.omit(userRoleInformation, [
        "role",
      ]);

      // All location keys from userRoleInformation
      let userRoleInfomrationLocationKeys = Object.keys(
        userRoleInformationLocationObject
      );
      ...
      for (
        ...
        userRoleInfomrationLocationKeys.length;
        pointerToUserRoleInfromationLocationKeys++
      ) {
        const locationType =
          userRoleInfomrationLocationKeys[
            pointerToUserRoleInfromationLocationKeys
        ...
        if (
          ...
          if (locationType === "school") {
            ...
            if (
              !_.find(userProfile.userLocations, {
                type: "school",
                code: locationValue,
              })
            ) {
      ...
      if (
        ...
        if (
          userProfile.userLocations.length !=
          userRoleInfomrationLocationKeys.length
        ) {
          updateUserProfileLocationInformation = true;
        }
      ...
      if (updateUserProfileLocationInformation) {
        ...
        userRoleInfomrationLocationKeys.forEach((requestedDataKey) => {
          if (
            gen.utils.checkIfValidUUID(
              userRoleInformationLocationObject[requestedDataKey]
            )
          ) {
            locationIds.push(
              userRoleInformationLocationObject[requestedDataKey]
        ...
        if (locationIds.length > 0) {
          ...
          let entityData = await userProfileService.locationSearch(
            locationQuery
          );
          if (entityData.success) {
      ...
      return resolve({
        ...
        profileMismatchFound:
          updateUserProfileLocationInformation ||
          updateUserProfileRoleInformation
            ? true
            : false,
        data: userProfile,
      });
